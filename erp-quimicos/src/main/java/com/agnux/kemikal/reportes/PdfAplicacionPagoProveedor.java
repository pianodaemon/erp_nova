...
public class PdfAplicacionPagoProveedor {
    ...
    public PdfAplicacionPagoProveedor(String fileout, String ruta_imagen, String razon_soc_empresa,  ArrayList<HashMap<String, String>> listaPagos) throws DocumentException {
        ...
        try {
            ...
            float [] widths = {2f, 2.5f, 4.5f, 2.5f, 3f, 3f,2.5f, 1f, 2.5f};
            ...
            if(listaPagos.size() > 0){
                ...
                no_transaccion = listaPagos.get(0).get("numero_transaccion");
                fecha_pago = listaPagos.get(0).get("fecha_pago");
                simbolo_moneda_pago = listaPagos.get(0).get("simbolo_moneda_pago");
            }
            ...
            if(listaPagos.size() > 0){
                ...
                for (int x=0; x<=listaPagos.size()-1;x++){
                    ...
                    //sumar cantidades
                    suma_montos_aplicados_en_mn += Double.parseDouble(registro.get("monto_aplicado_factura"));
                    simbolo_moneda = registro.get("simbolo_moneda_factura");
                    
                    //columna moneda
                    cell= new PdfPCell(new Paragraph(registro.get("moneda_pago"),smallFont));
                    cell.setHorizontalAlignment (Element.ALIGN_LEFT);
                    cell.setBorder(0);
                    table.addCell(cell);
                    ...
                    cell= new PdfPCell(new Paragraph(registro.get("cheque_referencia"),smallFont));
                    cell.setHorizontalAlignment (Element.ALIGN_LEFT);
                    cell.setBorder(0);
                    table.addCell(cell);
            ...
            Logger.getLogger(PdfAplicacionPagoProveedor.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    static class HeaderFooter extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            ColumnText.showTextAligned(writer.getDirectContent(),Element.ALIGN_CENTER, new Phrase("Reporte de Aplicación de Pagos a Proveedores",largeBoldFont),document.getPageSize().getWidth()/2, document.getPageSize().getTop()-38, 0);
