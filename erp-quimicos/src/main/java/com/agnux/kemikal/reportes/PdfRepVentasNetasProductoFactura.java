 ...
 */
public class PdfRepVentasNetasProductoFactura {
    ...
    public PdfRepVentasNetasProductoFactura(Integer tipo, Integer tipo_reporte, ArrayList<HashMap<String, String>> sumatorias, ArrayList<HashMap<String, String>> lista_ventas, String fecha_inicial, String fecha_final, String razon_social_empresa, String fileout) {
        ...
        try {
            ...
            float [] widths1 = {1.7f,5f,1.7f,2f,1.7f,1.5f,0.5f,1.5f,0.5f,2f,1.5f,1.5f,0.5f,2f,2f,2f,2f};
            float [] widths2 = {1.7f,5f,1.7f,2f,1.7f,1.5f,0.5f,1.5f,0.5f,2f,1.5f,1.5f,2f,2f,2f};
            
            String[] titulos1 = {"CODIGO", "PRODUCTO", "FACTURA", "FECHA", "UNIDAD", "CANTIDAD", "", "PRECIO U.", "", "V. NETA.", "MONEDA", "T. C.","","COSTO","% POND","% MOP","% M_MOP"};
            String[] titulos2 = {"CODIGO", "PRODUCTO", "FACTURA", "FECHA", "UNIDAD", "CANTIDAD", "", "PRECIO U.", "", "V. NETA.", "MONEDA", "T. C.","% POND","% MOP","% M_MOP"};
            ...
            cell = new PdfPCell(new Paragraph("Cliente: " + cliente, smallFont));
            cell.setColspan(18);
            cell.setBorder(0);
            tabla.addCell(cell);
            ...
            for (int j = 0; j <= lista_ventas.size() - 1; j++) {     //inicia for para recorrer las filas de la lista
                ...
                if (cliente.equals(registro.get("razon_social"))) {   //compara si todos los registros de la razon social
                    ...
                    cell = new PdfPCell(new Paragraph("Total por Cliente: ", smallFont));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setBorder(1);
                    tabla.addCell(cell);
                    ...
                    cell = new PdfPCell(new Paragraph("Cliente: " + registro.get("razon_social"), smallFont));
                    cell.setColspan(18);
                    cell.setBorder(0);
                    tabla.addCell(cell);
        ...
        try {
            for (int j = 0; j <= array_sumatoria.size() - 1; j++) {     //inicia for para recorrer las filas de la lista
                ...
                if (this.getTipo_reporte() == 1 || this.getTipo_reporte() == 4) {
                    if (registro.get("cliente").equals(cliente_producto)) {
    ...
    class HeaderFooter extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase(PdfRepVentasNetasProductoFactura.this.getEmpresa_emisora(), largeBoldFont), document.getPageSize().getWidth() / 2, document.getPageSize().getTop() - 25, 0);
            if(PdfRepVentasNetasProductoFactura.this.getTipo()==1){
                ...
                ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase("Reporte Comercial por Cliente.", largeBoldFont), document.getPageSize().getWidth() / 2, document.getPageSize().getTop() - 38, 0);
            }
            ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase(PdfRepVentasNetasProductoFactura.getFecha_reporte(), largeFont), document.getPageSize().getWidth() / 2, document.getPageSize().getTop() - 50, 0);
