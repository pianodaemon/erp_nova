...
public class n2t {
    ...
    public String num_letracdmm;
    public String num_letramcdmm;
    public String [] temp;
    ...
    private String decena(int numero){
        if (numero >= 90 && numero <= 99){
            ...
            switch (numero){
                case 10: num_letra = "diez ";
                         break;
                case 11: num_letra = "once ";
                         break;
                case 12: num_letra = "doce ";
                         break;
                case 13: num_letra = "trece ";
                         break;
                case 14: num_letra = "catorce ";
                         break;
                case 15: num_letra = "quince ";
                         break;
                case 16: num_letra = "dieciseis ";
                         break;
                case 17: num_letra = "diecisiete ";
                         break;
                case 18: num_letra = "dieciocho ";
                         break;
                case 19: num_letra = "diecinueve ";
                         break;
            }
        }
        else
...
//        temp =
    ...
    private String ciendecmillon(int numero){
        if (numero == 100000000)
            num_letracdmm = "cien millones";
        if (numero >= 100000000 && numero <1000000000){
            flag=1;
            num_letracdmm = centena(numero/1000000).concat(" millones ").concat(cienmiles(numero%1000000));
        }
        if (numero < 100000000)
            num_letracdmm = decmillon(numero);

        return num_letracdmm;
    }

    private String milciendecmillon(BigInteger numero){
		BigInteger uno = new BigInteger("1000000000");
		BigInteger dos = new BigInteger("10000000000");

		BigInteger unodivide = new BigInteger("1000000");
		BigInteger dosdivide = new BigInteger("1000000");

        if (numero.equals(uno))
            num_letramcdmm = "mil millones";
        if (numero.compareTo(uno) >= 0 && numero.compareTo(dos) < 0){
            flag=1;
            num_letramcdmm = decmiles(numero.divide(unodivide).intValue()).concat(" millones ").concat(cienmiles(numero.mod(dosdivide).intValue()));
        }
        if (numero.compareTo(uno) < 0)
            num_letramcdmm = ciendecmillon(numero.intValue());

        return num_letramcdmm;
    }

    public String convertirLetras(BigInteger numero){
        num_letras = milciendecmillon(numero);
        return num_letras;
    }
}
