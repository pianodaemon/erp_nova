$(function() {
	var config =  {
		...
		tituloApp: 'Reporte de Estado de Resultados' ,                 
		...
		getController: function(){
			return this.contextpath + "/controllers/ctbrepestadoresult";
	...
	$genera_PDF.click(function(event){
		...
		var input_json = config.getUrlForGetAndPost() + '/getPdfRepEstadoResultados/'+cadena+'/'+iu+'/out.json';
	...
	$busqueda_reporte.click(function(event){
		...
		$.post(restful_json_service,arreglo_parametros,function(entry){
			...
			var header_tabla = {
				descripcion			:'Cuenta',
				saldo_mensual	    :'Mensual',
				porcentajeuno		:'Mensual %',
				saldo_anual       :'Anual',
				porcentajedos		:'Anual %',
			...
			if(parseInt(body_tabla.length)>0){
				...
				for(var i=0; i<body_tabla.length; i++){
					...
					html_reporte +='<td align="right">'+$(this).agregar_comas(body_tabla[i]["saldo_mensual"])+'</td>';
					html_reporte +='<td align="left">'+body_tabla[i]["porcentajeuno"]+'</td>';
					html_reporte +='<td align="right">'+$(this).agregar_comas(body_tabla[i]["saldo_anual"])+'</td>';
					html_reporte +='<td align="left">'+body_tabla[i]["porcentajedos"]+'</td>';
					html_reporte +='</tr>';
				}
