$(function() {
	...
	*/
	...
	$sumatoria_asientos = function($grid_cuentas){
		...
		$('#forma-ctbdefasientos-window').find('#suma_debe').html($(this).agregar_comas(parseFloat(suma_debe).toFixed(2)));
		$('#forma-ctbdefasientos-window').find('#suma_haber').html($(this).agregar_comas(parseFloat(suma_haber).toFixed(2)));
	...
	var $agrega_tr = function($grid_cuentas, id_det, id_cta, cta, descripcion, tipo){
		...
		if(parseInt(encontrado)<=0){
			...
			trr = '<tr>';
				...
				trr += '<td class="grid1" style="font-size:11px;  border:1px solid #C1DAD7;" width="70">';
					trr += '<select name="select_mov" id="select_mov'+ noTr +'" style="width:68px;"></select>';
				trr += '</td>';
			trr += '</tr>';
		...
		if($cuenta.val().trim()!='' || $scuenta.val().trim()!='' || $sscuenta.val().trim()!='' || $ssscuenta.val().trim()!='' || $sssscuenta.val().trim()!=''){
			...
			$.post(input_json,$arreglo,function(entry){
				...
				if(parseInt(entry['Cta'].length)>0){
					...
					$agrega_tr($grid_cuentas, id_det, id_cta, cta, descripcion, tipo);
				}else{
	...
	var carga_formaCC00_for_datagrid00 = function(id_to_show, accion_mode){
		...
		if(accion_mode == 'cancel'){
			...
			$(this).modalPanel_ctbdefasientos();
			$('#forma-ctbdefasientos-window').css({ "margin-left": -470, 	"margin-top": -255 });
			...
			var $accion = $('#forma-ctbdefasientos-window').find('input[name=accion]');
			var $identificador = $('#forma-ctbdefasientos-window').find('input[name=identificador]');
			var $estatus = $('#forma-ctbdefasientos-window').find('input[name=estatus]');
			
			var $select_sucursal = $('#forma-ctbdefasientos-window').find('select[name=select_sucursal]');
			var $select_mes = $('#forma-ctbdefasientos-window').find('select[name=select_mes]');
			var $select_anio = $('#forma-ctbdefasientos-window').find('select[name=select_anio]');
			
			var $no_poliza = $('#forma-ctbdefasientos-window').find('input[name=no_poliza]');
			var $fecha = $('#forma-ctbdefasientos-window').find('input[name=fecha]');
			var $select_moneda = $('#forma-ctbdefasientos-window').find('select[name=select_moneda]');
			
			var $select_tipo = $('#forma-ctbdefasientos-window').find('select[name=select_tipo]');
			var $select_concepto = $('#forma-ctbdefasientos-window').find('select[name=select_concepto]');
			
			var $id_cta = $('#forma-ctbdefasientos-window').find('input[name=id_cta]');
			var $cuenta = $('#forma-ctbdefasientos-window').find('input[name=cuenta]');
			var $scuenta = $('#forma-ctbdefasientos-window').find('input[name=scuenta]');
			var $sscuenta = $('#forma-ctbdefasientos-window').find('input[name=sscuenta]');
			var $ssscuenta = $('#forma-ctbdefasientos-window').find('input[name=ssscuenta]');
			var $sssscuenta = $('#forma-ctbdefasientos-window').find('input[name=sssscuenta]');
			var $descripcion_cuenta = $('#forma-ctbdefasientos-window').find('input[name=descripcion_cuenta]');
			
			var $descripcion_pol = $('#forma-ctbdefasientos-window').find('textarea[name=descripcion_pol]');
			var $referencia = $('#forma-ctbdefasientos-window').find('input[name=referencia]');
			
			var $btn_contabilizar = $('#forma-ctbdefasientos-window').find('#btn_contabilizar');
			var $btn_cancelar = $('#forma-ctbdefasientos-window').find('#btn_cancelar');
			
			var $busca_cuenta_contble = $('#forma-ctbdefasientos-window').find('#busca_cuenta_contble');
			var $agrega_cuenta_contble = $('#forma-ctbdefasientos-window').find('#agrega_cuenta_contble');
