$(function() {
	var config =  {
		...
		getController: function(){
			return this.contextpath + "/controllers/repremision";
	...
	var $remision = $('#lienzo_recalculable').find('table#busqueda tr td').find('input[name=remision]');
	var $select_estatus = $('#lienzo_recalculable').find('table#busqueda tr td').find('select[name=estatus]');
	...
	var $genera_reporte_remision = $('#lienzo_recalculable').find('table#busqueda tr td').find('input[value$=Generar_PDF]');
	var $busqueda_reporte_remision= $('#lienzo_recalculable').find('table#busqueda tr td').find('input[value$=Buscar]');
	var $div_reporte_remision= $('#lienzo_recalculable').find('#divreporteremision');
	...
	var hmtl_estatus;
	hmtl_estatus += '<option value="4" selected="yes">[-- Seleccionar --]</option>';
	hmtl_estatus += '<option value="0">No Pagado</option>';
	hmtl_estatus += '<option value="1">Pagado</option>';
	hmtl_estatus += '<option value="2">Facturado</option>';
	hmtl_estatus += '<option value="3">Cancelado</option>';
	$select_estatus.append(hmtl_estatus);
	...
	$genera_reporte_remision.click(function(event){
		event.preventDefault();
		var busqueda = $select_opciones.val()+"___"+$remision.val()+"___"+$cliente.val()+"___"+$fecha_inicial.val()+"___"+$fecha_final.val()+"___"+$select_estatus.val();
		var input_json = config.getUrlForGetAndPost() + '/get_genera_reporte_remision/'+busqueda+'/'+config.getUi()+'/out.json';
		window.location.href=input_json;
	});
		
	
	$busqueda_reporte_remision.click(function(event){
		event.preventDefault();
		$div_reporte_remision.children().remove();
		...
		if(fecha_inicial != "" && fecha_final != ""){
			...
			var restful_json_service = config.getUrlForGetAndPost() + '/getRemision.json'
			var cliente="";
			$.post(restful_json_service,arreglo_parametros,function(entry){
				...
				for(var i=0; i<body_tabla.length; i++){
					...
					html_reporte +='<tr>';
					html_reporte +='<td align="left" >'+body_tabla[i]["remision"]+'</td>'; 
					html_reporte +='<td align="left" >'+orden_compra+'</td>'; 
					html_reporte +='<td align="left" >'+body_tabla[i]["fecha_remision"]+'</td>'; 
				...
				$div_reporte_remision.append(html_reporte); 
	...
	$(this).aplicarEventoKeypressEjecutaTrigger($select_opciones, $busqueda_reporte_remision);
	$(this).aplicarEventoKeypressEjecutaTrigger($remision, $busqueda_reporte_remision);
	$(this).aplicarEventoKeypressEjecutaTrigger($cliente, $busqueda_reporte_remision);
	$(this).aplicarEventoKeypressEjecutaTrigger($fecha_inicial, $busqueda_reporte_remision);
	$(this).aplicarEventoKeypressEjecutaTrigger($fecha_final, $busqueda_reporte_remision);
