$(function() {
	...
	var $new_logvehiculos = $('#barra_acciones').find('.table_acciones').find('a[href*=new_item]');
	...
	$('#barra_titulo').find('#td_titulo').append('Cat&aacute;logo de Vehiculos');
	...
	var to_make_one_search_string = function(){
		...
		var signo_separador = "=";
                ...
                valor_retorno += "numero_economico" + signo_separador + $busqueda_num_economico.val() + "|";
	...
	//nuevas vehiculos
	$new_logvehiculos.click(function(event){
		event.preventDefault();
		var id_to_show = 0;
		$(this).modalPanel_LogVehiculos();   //llamada al plug in 
		...
		$('#forma-logvehiculos-window').css({"margin-left": -300, 	"margin-top": -200});
		$forma_selected.prependTo('#forma-logvehiculos-window');
		$forma_selected.find('.panelcito_modal').attr({id : 'panelcito_modal' + id_to_show , style:'display:table'});
		$tabs_li_funxionalidad();
                
		//campos de la vista
		var $campo_id = $('#forma-logvehiculos-window').find('input[name=identificador]'); 
                //alert($campo_id);
		var $marca = $('#forma-logvehiculos-window').find('input[name=marca]');
                var $numero_economico = $('#forma-logvehiculos-window').find('input[name=numero_economico]');
		...
		var respuestaProcesada = function(data){
			if ( data['success'] == "true" ){
				jAlert("El Vehiculo fue dado de alta con exito", 'Atencion!');
	...
	var carga_formaCC00_for_datagrid00 = function(id_to_show, accion_mode){
		...
		if(accion_mode == 'cancel'){
			...
			jConfirm('Realmente desea eliminar el Vehiculo seleccionado', 'Dialogo de confirmacion', function(r) {
                            if (r){
                                $.post(input_json,$arreglo,function(entry){
                                        if ( entry['success'] == '1' ){
                                                jAlert("El Vehiculo fue eliminado exitosamente", 'Atencion!');
                                                $get_datos_grid();
                                        }
                                        else{
                                                jAlert("El Vehiculo no pudo ser eliminado", 'Atencion!');
                                        }
                                },"json");
                            }
			});
			...
			$('#forma-logvehiculos-window').css({"margin-left": -300, 	"margin-top": -200});
                        ...
                        var $marca = $('#forma-logvehiculos-window').find('input[name=marca]');
                        var $numero_economico = $('#forma-logvehiculos-window').find('input[name=numero_economico]');
                            $numero_economico.attr("readonly",true);
			...
			if(accion_mode == 'edit'){
				...
				// aqui van los campos de editar
                                $campo_id.attr({'value' : entry['Vehiculo']['0']['id']});
                                $marca.attr({'value' : entry['Vehiculo']['0']['marca']});      
				$numero_economico.attr({'value' : entry['Vehiculo']['0']['numero_economico']});  
